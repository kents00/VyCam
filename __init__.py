...
class POVCamera:
    def __init__(self):
        ...
        self.camera_object = bpy.data.objects.new(
            name="Camera", object_data=self.camera_data)
...
class Vycam_pg(PropertyGroup):
    ...
    focal_length_presets: EnumProperty(
        ...
        items=[
            ...
            ("F4", "85mm+ - Short telephoto",
             "Used for street photography and portraits"),
            ("F5", "135mm+ - Medium telephoto",
             "Used for sports, wildlife and  action"),
            ("F6", "200mm+ - Macro", "Used for closed up shots"),
            ("F7", "300mm+ - Super telephoto",
             "Used for sports from a distance, nature and astronomy"),
            ("F8", "Custom", ""),
        ]
    )
...
class Vycam_op(Operator):
    ...
    def execute(self, context):
        ...
        if custom_scene_property.focal_length_presets == "F1":
            ...
            bpy.context.object.data.lens = int(
                custom_scene_property.custom_focal_length)
            bpy.context.object.data.ortho_scale = int(
                custom_scene_property.custom_focal_length)
